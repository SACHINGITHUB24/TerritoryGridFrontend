...
export default function TerritoryGrid({ userId, onStatsUpdate, onActivity, onLeaderboard }: Props) {
  ...
  useEffect(() => {
    ...
    socket.on('init', (blocks: { id: number; ownerId: string; isBot?: boolean }[]) => {
      console.log('📦 init received:', blocks.length, 'blocks')
      const map: Record<number, string> = {}
      blocks.forEach((b) => {
        map[b.id] = b.ownerId
      })
    ...
    socket.on('block_updated', (data: { blockId: number; ownerId: string; isBot?: boolean; x?: number; y?: number }) => {
      console.log('🎯 block_updated received:', data)
      
      const { blockId, ownerId } = data
      
      if (typeof blockId !== 'number' || typeof ownerId !== 'string') {
        console.error('Invalid block_updated data:', data)
        return
      }
      ...
      const x = data.x ?? blockId % GRID_SIZE
      const y = data.y ?? Math.floor(blockId / GRID_SIZE)
      onActivity({ ownerId, x, y, color: getColor(ownerId, userId) })
  ...
  const handleBlockClick = (index: number) => {
    const x = index % GRID_SIZE
    const y = Math.floor(index / GRID_SIZE)
    console.log('👆 Clicked block:', index, { x, y, userId })
    socketRef.current?.emit('claim_block', { blockId: index, x, y, userId })
  }
